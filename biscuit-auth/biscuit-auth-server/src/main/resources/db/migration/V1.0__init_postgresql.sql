...
comment
on column t_oauth_client_details.client_authentication_methods is '客户端支持的认证方式';

comment
on column t_oauth_client_details.scopes is '指定客户端申请的权限范围,可选值包括read,write,trust;';
...
comment
on column t_oauth_client_details.post_logout_redirect_uri is 'postLogoutRedirectUri,可为空';
...
comment
on column t_oauth_client_details.access_token_format is 'accessToken的类型，reference-不透明的token，self-contained-jwt类型的token';


comment
on column t_oauth_client_details.reuse_refresh_token is '是否复用刷新令牌 为true则复用刷新令牌（refresh token），为false则签发新的刷新令牌。';
