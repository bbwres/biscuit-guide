...
public interface RoleAccountMapper extends BatchBaseMapper<RoleAccountEntity> {
    ...
    /**
     * 根据账号id查询出角色信息，支持按客户端id筛选（有租户过滤）
     *
     * @param accountId
     * @param status
     * @param clientId  可选，为空则不过滤
     * @return
     */
    List<RoleEntity> findByAccountIdAndClientId(@Param("accountId") String accountId, @Param("status") DataStatusEnum status, @Param("clientId") String clientId);
